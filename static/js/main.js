...
function applyResultDecorations(task, result) {
  ...
  if (task.algorithm === 'mst_weighted' && fr?.mst_edges) {
    fr.mst_edges.forEach(([u, v]) => updateEdgeColor(u, v, PINK));
  }

  if (task.algorithm === 'shortest_paths_weighted' && fr?.paths) {
...
async function runTask() {
  ...
  if (task.input_type === 'matrix' || task.input_type === 'weighted_matrix') {
  ...
  lastMatrix = (task.input_type === 'matrix' || task.input_type === 'weighted_matrix') ? currentMatrix : null;
